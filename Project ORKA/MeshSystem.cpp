...
void generateCPUMeshFromLoaderQueue(MeshSystem& meshSystem) {
	...
	//debugPrint("Generating Mesh!");
	//CPUMesh mesh;

	//mesh.name = meshSystem.meshLoadQueue.front().name;

	////create the mesh
	//mesh.primitiveMode = meshSystem.meshLoadQueue.front().primitiveMode;
	//Assimp::Importer Importer;
	//const aiScene * scene = Importer.ReadFile(meshSystem.meshLoadQueue.front().path,
	//	aiProcess_CalcTangentSpace |
	//	aiProcess_GenSmoothNormals |
	//	aiProcess_JoinIdenticalVertices |
	//	aiProcess_SortByPType |
	...
	//			for (Index vertexIndex = 0; vertexIndex < scene->mMeshes[0]->mNumVertices; vertexIndex++) {
	//				glm::vec3 vertex;
	//				vertex.x = scene->mMeshes[0]->mVertices[vertexIndex].x;
	//				vertex.y = scene->mMeshes[0]->mVertices[vertexIndex].y;
	//				vertex.z = scene->mMeshes[0]->mVertices[vertexIndex].z;
	//				mesh.vertices.push_back(vertex);

	//				glm::vec2 texCoord;
	//				texCoord.x = scene->mMeshes[0]->mTextureCoords[0][vertexIndex].x;
	//				texCoord.y = scene->mMeshes[0]->mTextureCoords[0][vertexIndex].y;
	//				mesh.uvs.push_back(texCoord);
	//			}

	//			if (scene->mMeshes[0]->HasFaces()) {
	//				for (Index faceIndex = 0; faceIndex < scene->mMeshes[0]->mNumFaces; faceIndex++) {
	//					for (Index indexIndex = 0; indexIndex < scene->mMeshes[0]->mFaces->mNumIndices; indexIndex++) {
	//						mesh.indices.push_back(scene->mMeshes[0]->mFaces[faceIndex].mIndices[indexIndex]);
	//					}
	//				}
	//			}
	//			else error = true;
	...
	//}
	//else error = true;
	//if (error) std::cout << "The " << meshSystem.meshLoadQueue.front().name << " model could not be loaded. (" << meshSystem.meshLoadQueue.front().path << ")" << std::endl;
	//
	////remove entry from loader queue
	//meshSystem.meshLoadQueue.pop_front();

	////add to upload queue
	//addMeshToUploadQueue(meshSystem, mesh);
	//debugPrint("Generated Mesh!");
}

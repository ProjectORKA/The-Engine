...
public:
	...
	constexpr friend void swap(BasicString<T>& first, BasicString<T>& second) noexcept {
		if (first.isSmall() && second.isSmall()) {
			...
			small
			->
			m_Data = notSmall->m_Data;
			small
			->
			m_Capacity = notSmallCap;
...
private:
	constexpr static auto SSO_SIZE = 2 * sizeof(usize);
	constexpr static auto NB_CHAR_SMALL = SSO_SIZE / sizeof(T) - 1;
	constexpr static auto NB_ELEMENTS = NB_CHAR_SMALL + 1;
	constexpr static T NULL_TERMINATOR = '\0';
	constexpr static T DEFAULT_GROW_SIZE = 2;
...
constexpr FORCEINLINE BasicString<T>::~BasicString() noexcept {
	T* data = this->m_Data;
	if (data && !this->isSmall()) { Utils::freeMemory(data); }
}
...
constexpr FORCEINLINE void BasicString<T>::append(const T* str, usize sz) {
	...
	for (usize i = 0; i < sz; i++) { this->m_Data[len++] = str[i]; }
...
template <typename T>
constexpr FORCEINLINE void BasicString<T>::pushBack(T ch) { return this->append(ch); }
...
constexpr void BasicString<T>::insert(usize pos, T ch) {
	...
	if (size + 2 >= cap) {
		const bool isSmall = this->isSmall();
		const usize nCap = cap * DEFAULT_GROW_SIZE;
...
constexpr void BasicString<T>::insert(usize pos, const T* str, usize otherSize) {
	...
	if (size + otherSize + 1 >= cap) {
		const bool isSmall = this->isSmall();
		const usize nCap = (size + otherSize) * DEFAULT_GROW_SIZE;
		this->m_Data = Utils::allocate<T>(nCap);
...
template <typename T>
constexpr FORCEINLINE void BasicString<T>::erase(iterator itr, const usize count) noexcept {
	const usize index = itr.getPointer() - this->m_Data;
...
template <typename T>
constexpr FORCEINLINE void BasicString<T>::reallocate(const usize nCap) {
	const bool isSmall = this->isSmall();
	T* oldData = this->m_Data;
	this->m_Data = Utils::allocate<T>(nCap);
...
constexpr FORCEINLINE BasicString<T>::BasicString(const BasicString<T>& other)
	...
	auto sz = this->m_Length;
	if (other.isSmall()) { this->m_Data = m_Buffer; }
	else {

...
UInt getFileSize(const Path& path) { return file_size(path); }
...
Path makeAbsolute(const Path& path) { return absolute(path); }
Bool doesPathExist(const Path& path) { return exists(makeAbsolute(path)); }
...
FileTime lastWrittenTime(const Path& path) { return last_write_time(path); }
...
void copyFile(const Path& source, const Path& destination) {
	logDebug(
		String("Copying file (").append(source.string()).append(") to (").append(destination.string()).append(")"));

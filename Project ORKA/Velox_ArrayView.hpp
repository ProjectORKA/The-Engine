...
public:
	...
	constexpr FORCEINLINE void removeSuffix(const usize skip) { m_Size -= skip; }
	...
	constexpr FORCEINLINE T& operator[](const usize i) noexcept { return this->at(i); }

	constexpr FORCEINLINE const T& operator[](const usize i) const noexcept { return this->at(i); }
	...
	constexpr const_iterator cbegin() const noexcept { return CIterator(m_Data); }

	constexpr const_iterator cend() const noexcept { return CIterator(m_Data + m_Size); }
...
template <typename T>
constexpr FORCEINLINE ArrayView<T>::ArrayView(T* data, const usize size) noexcept

...
Bool TripleNinePlayer::isCollidingWithGround() const { return location.z <= 0; }
...
void TripleNinePlayer::calculateHeadPosition(Window& window, const Float delta) {
	...
	//bob
	Float bobTarget = 0;
	if (isMoving && onGround) bobTarget = sin(lerp(1, headBobSpeed, 0.5) * 2 * walkCycle) * actualSpeed *
		headBobIntensity;
	...
	auto leanOffset = Vec2(lowerChestHeight * sin(fullBodyLeanAngle),
	                       lowerChestHeight * cos(fullBodyLeanAngle) - lowerChestHeight);
	...
	leanOffset += Vec2(lowerChestToEyeDistance * sin(fullLeanAngle),
	                   lowerChestToEyeDistance * cos(fullLeanAngle) - lowerChestToEyeDistance);

	//apply changes to camera
	camera.setRotation(DVec3(camera.getRotationX(), -(fullBodyLeanAngle + upperBodyLean * upperBodyLeanHeadTiltFactor),
	                         camera.getRotationZ()));

	//calculate 3d position of all offsets
	const Vec3 sway3D = -sway * normalize(camera.rightVector * Vec3(1, 1, 0));
	const Vec3 lean3D = leanOffset.x * camera.rightVector + Vec3(0, 0, leanOffset.y);
...
void TripleNineSimulation::createEnemy() {
	...
	enemies.back().id = enemies.size() - 1;
}

void TripleNineRenderer::connect(TripleNineSimulation& sim) { this->sim = &sim; }

void TripleNineRenderer::update(Window& window) { player.update(window); }
...
void TripleNineRenderer::inputEvent(Window& window, const InputEvent input) {
	if (input == enter) {
		if (!window.capturing)window.captureCursor();
			...
			if (window.renderer.idFramebuffer.objectID != -1) sim->enemies[window.renderer.idFramebuffer.objectID].
				die();
		}
	}
...
void TripleNineRenderer::render(ResourceManager& resourceManager, Window& window, TiledRectangle area) {
	...
	r.textRenderSystem.render(resourceManager, r, "on Ground: " + toString(player.onGround),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r,
	                          "horizontal speed: " + toString(length(player.velocity * Vec3(1, 1, 0))),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "is moving: " + toString(player.isMoving),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "full body lean: " + toString(player.fullBodyLeanAngle),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "upper body lean: " + toString(player.upperBodyLeanAngle),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "target id: " + toString(r.idFramebuffer.objectID),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "vertical speed: " + toString(player.velocity.z),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "double jump: " + toString(player.doubleJumpCharge),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "max jump height: " + toString(player.debugCurrentMaxJumpHeight),
	                          Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "fps: " + toString(1 / r.time.delta), Vec2(spacing, i++ * spacing));
	r.textRenderSystem.render(resourceManager, r, "cameraForwardVector: " + toString(player.camera.forwardVector),
	                          Vec2(spacing, i++ * spacing));
}

void TripleNineRenderer::renderInteractive(ResourceManager& resourceManager, Window& window,
                                           const TiledRectangle area) {
	Renderer& r = window.renderer;

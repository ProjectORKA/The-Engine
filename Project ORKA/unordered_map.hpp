...
namespace ska {
	namespace detailv10 {
		...
		struct sherwood_v10_entry {
			...
			using EntryPointer = typename std::allocator_traits<typename std::allocator_traits<Allocator>::template
				rebind_alloc<sherwood_v10_entry>>::pointer;
			...
			static EntryPointer* empty_pointer() {
				static EntryPointer result[3] = {EntryPointer(nullptr) + static_cast<ptrdiff_t>(1), nullptr, nullptr};
				return result + 1;
			}
		};
		...
		template <typename T, typename FindKey, typename ArgumentHash, typename Hasher, typename ArgumentEqual, typename
		          Equal, typename ArgumentAlloc, typename EntryAlloc, typename BucketAllocator>
		...
		public:
			...
			explicit sherwood_v10_table(const size_type bucket_count, const ArgumentHash& hash = ArgumentHash(),
			                            const ArgumentEqual& equal = ArgumentEqual(),
			                            const ArgumentAlloc& alloc = ArgumentAlloc())
				: EntryAlloc(alloc), Hasher(hash), Equal(equal), BucketAllocator(alloc) { rehash(bucket_count); }
			...
			sherwood_v10_table(It first, It last, size_type bucket_count = 0, const ArgumentHash& hash = ArgumentHash(),
			                   const ArgumentEqual& equal = ArgumentEqual(),
			                   const ArgumentAlloc& alloc = ArgumentAlloc())
				: sherwood_v10_table(bucket_count, hash, equal, alloc) { insert(first, last); }
			...
			template <typename It>
			sherwood_v10_table(It first, It last, size_type bucket_count, const ArgumentHash& hash,
			...
			sherwood_v10_table(const sherwood_v10_table& other, const ArgumentAlloc& alloc)
				: EntryAlloc(alloc), Hasher(other), Equal(other), BucketAllocator(alloc),
			...
			sherwood_v10_table(sherwood_v10_table&& other) noexcept
				: EntryAlloc(std::move(other)), Hasher(std::move(other)), Equal(std::move(other)),
				  BucketAllocator(std::move(other)), _max_load_factor(other._max_load_factor) { swap_pointers(other); }

			sherwood_v10_table(sherwood_v10_table&& other, const ArgumentAlloc& alloc) noexcept
				: EntryAlloc(alloc), Hasher(std::move(other)), Equal(std::move(other)), BucketAllocator(alloc),
				  _max_load_factor(other._max_load_factor) { swap_pointers(other); }
			...
			sherwood_v10_table& operator=(const sherwood_v10_table& other) {
				...
				if (AllocatorTraits::propagate_on_container_copy_assignment::value) {
					if (static_cast<EntryAlloc&>(*this) != static_cast<const EntryAlloc&>(other) || static_cast<
						BucketAllocator&>(*this) != static_cast<const BucketAllocator&>(other)) {
						reset_to_empty_state();
					}
					AssignIfTrue<EntryAlloc, AllocatorTraits::propagate_on_container_copy_assignment::value>()(
						*this, other);
					AssignIfTrue<
						BucketAllocator, BucketAllocatorTraits::propagate_on_container_copy_assignment::value>()(
						*this, other);
				}
				_max_load_factor = other._max_load_factor;
			...
			sherwood_v10_table& operator=(sherwood_v10_table&& other) noexcept {
				...
				if (AllocatorTraits::propagate_on_container_move_assignment::value) {
					...
					AssignIfTrue<EntryAlloc, AllocatorTraits::propagate_on_container_move_assignment::value>()(
						*this, std::move(other));
					AssignIfTrue<
						BucketAllocator, BucketAllocatorTraits::propagate_on_container_move_assignment::value>()(
						*this, std::move(other));
					swap_pointers(other);
				}
				else if (static_cast<EntryAlloc&>(*this) == static_cast<EntryAlloc&>(other) && static_cast<
					BucketAllocator&>(*this) == static_cast<BucketAllocator&>(other)) { swap_pointers(other); }
				else {
					clear();
					_max_load_factor = other._max_load_factor;
					rehash_for_other_container(other);
			...
			struct templated_iterator {
				...
				ValueType& operator*() const { return current_element->value; }
				ValueType* operator->() const { return std::addressof(current_element->value); }

				operator templated_iterator<const value_type>() const { return {current_element, current_bucket}; }
			};
			...
			const_iterator begin() const { return const_cast<sherwood_v10_table*>(this)->begin(); }
			...
			iterator find(const FindKey& key) {
				...
				EntryPointer* bucket = entries + static_cast<ptrdiff_t>(index);
			...
			const_iterator find(const FindKey& key) const { return const_cast<sherwood_v10_table*>(this)->find(key); }
			...
			void rehash(size_t num_buckets) {
				...
				*new_buckets = EntryPointer(nullptr) + static_cast<ptrdiff_t>(1);
				++new_buckets;
				...
				for (EntryPointer *it = new_buckets, *end = it + static_cast<ptrdiff_t>(num_buckets + 1); it != end; ++
				     it) {
			...
			convertible_to_iterator erase(const_iterator to_erase) {
				...
				while (*previous != to_erase.current_element) { previous = &(*previous)->next; }
			...
			convertible_to_iterator erase(const_iterator begin_it, const_iterator end_it) {
				while (begin_it.current_bucket != end_it.current_bucket) { begin_it = erase(begin_it); }
			...
			void clear() {
				if (!num_slots_minus_one)
					return;
				for (EntryPointer *it = entries, *end = it + static_cast<ptrdiff_t>(num_slots_minus_one + 1); it != end;
				     ++it) {
					for (EntryPointer e = *it; e;) {
						EntryPointer next = e->next;
			...
			size_type max_bucket_count() const { return (AllocatorTraits::max_size(*this) - 1) / sizeof(Entry); }
		...
		private:
			...
			void rehash_for_other_container(const sherwood_v10_table& other) { reserve(other.size()); }
	...
	class unordered_map
		...
		<
			...
			typename std::allocator_traits<A>::template rebind_alloc<typename std::allocator_traits<A>::template
				rebind_traits<detailv10::sherwood_v10_entry<std::pair<K, V>, A>>::pointer>
		> {
	...
	public:
		...
		friend bool operator!=(const unordered_map& lhs, const unordered_map& rhs) { return !(lhs == rhs); }
	...
	class unordered_set
		...
		<
			...
			typename std::allocator_traits<A>::template rebind_alloc<typename std::allocator_traits<A>::template
				rebind_traits<detailv10::sherwood_v10_entry<T, A>>::pointer>
		> {
		using Table = detailv10::sherwood_v10_table
		<
			T,
	...
	public:
		...
		friend bool operator!=(const unordered_set& lhs, const unordered_set& rhs) { return !(lhs == rhs); }
	};
} // end namespace ska

...
void renderMoonCrashAtmosphere(ResourceManager& resourceManager, Renderer& renderer, MoonCrashPlayer& player)
{
	...
	renderer.uniforms().cameraPos() = Vec4(Vec3(0, 0, (static_cast<Float>(player.chunkLocation.z) + player.camera.location.z) / static_cast<Float>(ULLONG_MAX)), 1);
...
void MoonCrashPlayer::update(Window& window)
{
	...
	if(floor(camera.location.x) != 0.0f)
	{
		if(floor(camera.location.x) > 0.0f)
		{

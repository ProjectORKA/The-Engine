	...
	if (heightmap.loaded) {
		...
		for (Int y = 0; y < TERRAIN_MAP_SIZE; y++) {
			for (Int x = 0; x < TERRAIN_MAP_SIZE; x++) {
				...
				else position.z = static_cast<Float>((heightmap.height[x][y] - heightmap.lowerLimit) / (heightmap.
					upperLimit - heightmap.lowerLimit));
		...
		for (UInt y = 0; y < TERRAIN_MAP_SIZE - 1; y++) {
			...
			for (UInt x = 0; x < TERRAIN_MAP_SIZE; x++) {
				...
				if ((x == (TERRAIN_MAP_SIZE - 1)) && (y < (TERRAIN_MAP_SIZE - 1))) mesh.indices.push_back(
					x + TERRAIN_MAP_SIZE * y);
			}
		}

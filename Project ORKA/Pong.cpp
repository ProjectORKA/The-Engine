...
void PongPlayer::aiInput(Vector<Ball>& balls, const Float deltaTime) {
	...
	if (ball) {
		...
		const Float predictedPosition = ball->position.y + ((position.x - ball->position.x) / ball->velocity.x) * ball->
			velocity.y;
...
void PongPlayer::update(Window& window) { position.y = clamp(position.y, -0.9f, 0.9f); }
...
void PongRenderer::render(ResourceManager& resourceManager, Window& window, TiledRectangle area) {
	...
	const Vec2 normalizedCursorPosition = Vec2(2) * ((Vec2(window.mousePosBotLeft) / Vec2(
		r.framebufferSystem.windowSize)) - Vec2(0.5));
	const Vec3 cursorWorldPos = inverse(r.uniforms().pMatrix()) * Vec4(normalizedCursorPosition, 0, 1);
	...
	const Float height = r.framebufferSystem.windowSize.y;
	const Float width = r.framebufferSystem.windowSize.x;

...
namespace ska {
	namespace detailv8 {
		...
		struct sherwood_v8_constants {
			...
			static int distance_from_metadata(const int8_t metadata) { return metadata & bits_for_distance; }
		...
		template <typename T, typename FindKey, typename ArgumentHash, typename Hasher, typename ArgumentEqual, typename
		          Equal, typename ArgumentAlloc, typename ByteAlloc, uint8_t BlockSize>
		...
		public:
			...
			explicit sherwood_v8_table(const size_type bucket_count, const ArgumentHash& hash = ArgumentHash(),
			...
			sherwood_v8_table(It first, It last, size_type bucket_count = 0, const ArgumentHash& hash = ArgumentHash(),
			                  const ArgumentEqual& equal = ArgumentEqual(),
			                  const ArgumentAlloc& alloc = ArgumentAlloc())
				: sherwood_v8_table(bucket_count, hash, equal, alloc) { insert(first, last); }
			...
			sherwood_v8_table& operator=(sherwood_v8_table&& other) noexcept {
				...
				else if (static_cast<ByteAlloc&>(*this) == static_cast<ByteAlloc&>(other)) { swap_pointers(other); }
				else {
					clear();
					_max_load_factor = other._max_load_factor;
					rehash_for_other_container(other);
			...
			public:
				...
				templated_iterator(const BlockPointer entries, const size_t index)
					: current(entries)
					  , index(index) { }
			...
			iterator find(const FindKey& key) {
				...
				while (true) {
					...
					const value_type& inMemKey = *reinterpret_cast<const value_type*>(&block->data[index_in_block]);
			...
			const_iterator find(const FindKey& key) const { return const_cast<sherwood_v8_table*>(this)->find(key); }
			...
			std::pair<iterator, bool> emplace(Key&& key, Args&&... args) {
				...
				index = hash_policy.index_for_hash(index, num_slots_minus_one);
				bool first = true;
				//printf("Insert [%d] = %d\n", key, typename value_type::first_type(std::forward<Args>(args)...));
			...
			void rehash(size_t num_items) {
				num_items = std::max(
					num_items, static_cast<size_t>(std::ceil(num_elements / static_cast<double>(_max_load_factor))));
				if (num_items == 0) {
					reset_to_empty_state();
					return;
				}
				...
				for (BlockPointer it = new_buckets, end = new_buckets + old_num_blocks; it != end; ++it) {
					for (int i = 0; i < BlockSize; ++i) {
						const int8_t metadata = it->control_bytes[i];
		...
		private:
			...
			struct LinkedListIt {
				...
				LinkedListIt(const size_t index, const BlockPointer block)
				...
				LinkedListIt next(sherwood_v8_table& table) const {
					const int8_t distance = jump_index();
				...
				void set_next(const int8_t jump_index) {
			...
			void grow() { rehash(std::max(static_cast<size_t>(10), 2 * bucket_count())); }

			size_t calculate_memory_requirement(const size_t num_blocks) {
			...
			void deallocate_data(BlockPointer begin, size_t num_slots_minus_one) {
				...
				auto as_byte_pointer = reinterpret_cast<unsigned char*>(begin);
		...
		template <typename T>
		struct AlignmentOr8Bytes<T, std::enable_if_t<alignof(T) >= 1>> {

...
public:
	struct InlinePair {
		...
		FORCEINLINE const K& key() const { return *reinterpret_cast<const K*>(key); }

		FORCEINLINE K& key() { return *reinterpret_cast<K*>(key); }

		FORCEINLINE const V& value() const { return *reinterpret_cast<const V*>(value); }

		FORCEINLINE V& value() { return *reinterpret_cast<V*>(value); }

		FORCEINLINE const K& first() const { return *reinterpret_cast<const K*>(key); }

		FORCEINLINE K& first() { return *reinterpret_cast<K*>(key); }

		FORCEINLINE const V& second() const { return *reinterpret_cast<const V*>(value); }

		FORCEINLINE V& second() { return *reinterpret_cast<V*>(value); }
	};
...
public:
	...
	struct Constants {
		static constexpr uint8 MAGIC_FOR_EMPTY = 0b11111111;
		static constexpr uint8 MAGIC_FOR_RESERVED = 0b11111110;
		static constexpr uint8 BITS_FOR_DIRECT_HIT = 0b10000000;
		static constexpr uint8 MAGIC_FOR_DIRECT_HIT = 0b00000000;
		static constexpr uint8 MAGIC_FOR_LIST_ENTRY = 0b10000000;
		static constexpr uint8 BITS_FOR_DISTANCE = 0b01111111;
		static constexpr uint32 NUM_JUMP_DISTANCES = 126;

		FORCEINLINE static uint8 distanceFromMetadata(const uint8 metadata) { return metadata & BITS_FOR_DISTANCE; }
	...
	struct Block {
		...
		constexpr FORCEINLINE void fillControlBytes(uint8 value) { Utils::MemSet(controlBytes, value, BlockSize); }

		constexpr FORCEINLINE value_type* getPair(uint32 idx) { return reinterpret_cast<value_type*>(pairs[idx].key); }

		constexpr FORCEINLINE const value_type* getPair(uint32 idx) const {
			return reinterpret_cast<const value_type*>(pairs[idx].key);
		}

		constexpr FORCEINLINE K* getKey(uint32 idx) { return reinterpret_cast<K*>(&pairs[idx].key); }

		constexpr FORCEINLINE const K* getKey(uint32 idx) const { return reinterpret_cast<K*>(&pairs[idx].key); }

		constexpr FORCEINLINE V* getValue(uint32 idx) { return reinterpret_cast<V*>(&pairs[idx].value); }

		constexpr FORCEINLINE const K* getValue(uint32 idx) const { return reinterpret_cast<V*>(&pairs[idx].value); }

		constexpr FORCEINLINE void destroy(const uint32 idx) {
...
public:
	...
	constexpr FORCEINLINE float loadFactor() const noexcept {
		return static_cast<float>(m_ElementsCount) / (m_SlotsCount + 1);
	}
	...
	constexpr FORCEINLINE const_iterator cbegin() const noexcept { return this->begin(); }

	constexpr FORCEINLINE iterator end() noexcept { return {m_Entries - 1, std::numeric_limits<usize>::max()}; }

	constexpr FORCEINLINE iterator end() const noexcept { return {m_Entries - 1, std::numeric_limits<usize>::max()}; }

	constexpr FORCEINLINE const_iterator cend() const noexcept { return this->end(); }
...
private:
	...
	template <typename U>
	constexpr FORCEINLINE usize hashObject(const U& key) noexcept { return static_cast<Hasher&>(*this)(key); }
	...
	constexpr FORCEINLINE bool comparesEqual(const L& lhs, const R& rhs) const noexcept { return lhs == rhs; }

	constexpr FORCEINLINE void grow() { return this->reinit(std::max(INITIAL_SIZE, 2 * this->bucketCount())); }

	constexpr FORCEINLINE usize numBucketsForReserve(const usize size) const noexcept {
		return static_cast<usize>(std::ceil(size / static_cast<double>(MAX_LOAD_FACTOR)));
	}

	constexpr FORCEINLINE usize calculateMemorySize(const usize blockCount) const noexcept {
...
template <typename K, typename V, typename H, typename HP, typename KE>
template <typename Key, typename... Args>
constexpr std::pair<iterator, bool> hashMapHelper<K, V, H, HP, KE>::emplace(Key&& key, Args&&... args) {
...
template <typename K, typename V, typename H, typename HP, typename KE>
template <typename Key, typename... Args>
constexpr std::pair<iterator, bool> hashMapHelper<K, V, H, HP, KE>::emplaceDirectHit(
	LinkedList block, Key&& key, Args&&... args) {
	if (this->isFull()) {
		this->grow();
	...
	for (LinkedList it = block;;) {
		...
		BlockType::construct(*freeBlock.second, std::move(keyValuePairPtr.first()),
		                     std::move(keyValuePairPtr.second()));
...
template <typename K, typename V, typename H, typename HP, typename KE>
template <typename Key, typename... Args>
constexpr std::pair<iterator, bool> hashMapHelper<K, V, H, HP, KE>::emplaceNewKey(
	LinkedList parent, Key&& key, Args&&... args) {
	if (this->isFull()) {
		this->grow();
...
template <typename K, typename V, typename H, typename HP, typename KE>
constexpr FORCEINLINE iterator hashMapHelper<K, V, H, HP, KE>::find(const K& key) const noexcept {
...
template <typename K, typename V, typename H, typename HP, typename KE>
constexpr FORCEINLINE IteratorProxy hashMapHelper<K, V, H, HP, KE>::erase(const const_iterator& toErase) noexcept {
...
constexpr FORCEINLINE void hashMapHelper<K, V, H, HP, KE>::clear() noexcept {
	...
	const usize slotsCount = m_SlotsCount + 1;
...
template <typename K, typename V, typename H, typename HP, typename KE>
constexpr FORCEINLINE void hashMapHelper<K, V, H, HP, KE>::reserve(const usize size) {
...
template <typename K, typename V, typename H, typename HP, typename KE>
constexpr void hashMapHelper<K, V, H, HP, KE>::deallocateData(BlockPointer begin, usize slotsCount) noexcept {
